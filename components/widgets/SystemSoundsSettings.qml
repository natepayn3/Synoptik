...
ColumnLayout {
    ...
    function previewSound(fileName) {
        let fullPath = Quickshell.shellDir.toString() + "/assets/" + fileName
        previewPlayer.source = fullPath
        previewPlayer.stop()
        previewPlayer.play()
    }
    ...
    // MASTER AUDIO TOGGLE CARD (Border Removed)
    Rectangle {
        Layout.fillWidth: true
        implicitHeight: 52
        ...
        RowLayout {
            ...
            // Custom Checkbox matching ClockWidget design system
            Rectangle {
                implicitWidth: 18
                implicitHeight: 18
                radius: Config.cornerRadius / 4
                color: Config.playSystemSounds ? Config.accent : "transparent"
                border.width: Config.playSystemSounds ? 0 : 1
                ...
                Text {
                    ...
                    font.pixelSize: 12
                    font.bold: true
                    visible: Config.playSystemSounds
                }
                ...
                MouseArea {
                    ...
                    onClicked: {
                        if (Config.isLoaded) {
                            Config.playSystemSounds = !Config.playSystemSounds
                        }
                    }
                }
            }
            ...
            ColumnLayout {
                ...
                Text {
                    text: Config.playSystemSounds ? "System sounds active" : "All shell sounds muted"
                    color: Config.textMuted
                    font.family: Config.sysFont
                    font.pixelSize: Config.size(Config.fontCaption)
                }
    ...
    // SELECTION CARD: WINDOW SOUNDS (Border Removed, Selection Border Preserved)
    Rectangle {
        Layout.fillWidth: true
        ...
        radius: Config.cornerRadius / 2
        border.width: 0
        opacity: Config.playSystemSounds ? 1.0 : 0.4
        enabled: Config.playSystemSounds
    ...
    // SELECTION CARD: NOTIFICATION SOUNDS (Border Removed, Selection Border Preserved)
    Rectangle {
        Layout.fillWidth: true
