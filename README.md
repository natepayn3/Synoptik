...
Building requires `git` and the `base-devel` group (for `fakeroot`,
`debugedit`, etc.), neither of which is installed by default on a minimal
Arch system. Two dependencies — `quickshell-git` and
`ttf-material-symbols-variable-git` — are AUR-only, so plain `makepkg -si`
can't resolve them on its own; install those first with an AUR helper such as
[`yay`](https://github.com/Jguer/yay) or [`paru`](https://github.com/Morganamilo/paru),
then `makepkg -si` will find them already satisfied:
